...
public class ReimburseService {
    ...
    public List<Reimbursements> getAllReimbursementsforEmployeeByType(String type, int employeeId) throws SQLException, ReimbursementNotFoundException {

        if(!"Travel".equals(type) &&  !"Food".equals(type) &&!"Lodging".equals(type) &&!"Other".equals(type) ){
            throw new ReimbursementNotFoundException("You have not entered an approved type. Please use Travel, Food, Lodging, or Other.");
        }

        return reimburseRepository.getReimbursementByTypeAndId(type, employeeId);
    }
    ...
    public List<Reimbursements> getAllReimbursementsByType(String type) throws SQLException, ReimbursementNotFoundException {
        if(!"Travel".equals(type) &&  !"Food".equals(type) &&!"Lodging".equals(type) &&!"Other".equals(type) ){
            throw new ReimbursementNotFoundException("You have not entered an approved type. Please use Travel, Food, Lodging, or Other.");
        }

        return reimburseRepository.getReimbursementByType(type);
    }

    public static void addReimbursement(Reimbursements reimbursement) throws SQLException, IllegalArgumentException, EmployeeIdNotFountException, DescriptionMissingException, TypeMissingException, WrongUpdateWordException {
        ...
        //Makes sure the employee provides the type of the reimbursement
        if(reimbursement.getType() == null || reimbursement.getType() == ""){
            throw new TypeMissingException("Please enter what type of reimbursement you are submitting");
        }
    // Make sure that the type entered meets expected types
        if(!"Travel".equals(reimbursement.getType()) &&  !"Food".equals(reimbursement.getType()) &&!"Lodging".equals(reimbursement.getType()) &&!"Other".equals(reimbursement.getType()) ){
            throw new WrongUpdateWordException("You have not entered an approved type. Please use Travel, Food, Lodging, or Other.");
        }
